...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const a = getProAudience('distributeurs');
  if (!a) return {};
  return {
    title: a.title[locale as Locale],
    description: a.subtitle[locale as Locale],
    alternates: getLocalizedAlternates('/pro/distributeurs', locale),
  };
}

export default async function DistributeursPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const audience = getProAudience('distributeurs');
