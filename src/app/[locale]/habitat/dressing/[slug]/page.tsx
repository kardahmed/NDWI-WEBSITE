...
export async function generateStaticParams() {
  ...
  return routing.locales.flatMap((locale) =>
    all.filter((p) => p.category === 'dressing').map((p) => ({ locale, slug: p.slug }))
  );
}
...
export default async function DressingDetailPage({
  ...
  const all = await fetchAllProducts();
  const product = all.find((p) => p.slug === slug && p.category === 'dressing');
  if (!product) notFound();

  return (
    <ProductDetail
      product={product}
      backHref="/habitat/dressing"
      backLabel={locale === 'ar' ? 'العودة إلى الخزائن' : 'Retour aux dressings'}
    />
  );
}
